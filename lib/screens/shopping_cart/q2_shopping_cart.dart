...
class _Q2ShoppingCartScreenState extends State<Q2ShoppingCartScreen> {
  ...
  void fetchCost() async{
    ...
    if(selected.country.toLowerCase() != "COLOMBIA" && selected.country.toLowerCase() != "colombia") {
      try {
        getMerchant(p.first.uid).then((value) async {
          setState(() {
            shippingCost = "Fetching...";
          });

          if (value.storeLocation.country.isNotEmpty &&
              value.storeLocation.zipCode.isNotEmpty &&
              selected.country.isNotEmpty && selected.zipCode.isNotEmpty) {
            shippingCost =
            await getDHLQoute(value.storeLocation.country.characters
                .take(2)
                .string, value.storeLocation.zipCode, selected.country,
                selected.zipCode);

            print(shippingCost);

            if(shippingCost != null){
              tEmeralds = tEmeralds +  double.parse(shippingCost).toInt();
            }
            ...
            ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text("Something wrong with address info!")));
          }
        });
      } catch (e) {
        setState(() {
          shippingCost = "Something went wrong...";
        });
      }
    }else{
      setState(() {
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    ...
                    user.emeralds != null && user.emeralds < tEmeralds
                        ? CumbiaButton(
                            onPressed: () {
                              Navigator.push(
                                  context,

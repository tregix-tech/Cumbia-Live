...
class _AudiencePageState extends State<AudiencePage> {
   ...
   // setState(() {
      ...
      /*print(user.addresses.length);
        for (int i = 0; i < user.addresses.length; i++) {
          ...
          addresses.add(user.addresses[i].address);
            ...
            itemBuilder: (context, position) {
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      productsList[position].productName != null? productsList[position].productName :"",
                      style: TextStyle(
  ...
  void _finishLive() {
    Map<String, dynamic> liveMap = ({
      "dates": {
        "startDate": liveAudience != null && liveAudience.isNotEmpty? liveAudience[0].startDate :"",
  ...
  Future<dynamic> getAddressData() async {
    ...
    await References.users.doc(user.id).get().then((value) => {
      dataAddress = value.data()['addresses'] as List,
    });

    addressesList.clear();
    for (var i = 0; i < dataAddress.length; i++) {
      addressesList.add(Address(
          country: dataAddress[i]['country'].toString(),
          city: dataAddress[i]['city'].toString(),
          address: dataAddress[i]['address'].toString(),
          zipCode:dataAddress[i]['zipCode'].toString(),
          isPrincipal: dataAddress[i]['isPrincipal'] as bool));

      addresses.add(dataAddress[i]['address'].toString());
    }
  ...
  int checkoutProducts() {
    ...
    for (int i = 0; i < checkoutList.length; i++) {
      setState(() {
        if(checkoutList[i].emeralds != null)
        emeralds += (checkoutList[i].emeralds) * checkoutList[i].unitsCheckout;
      });
    }


    if(shippingCost != null && shippingCost.isNotEmpty && !shippingCost.contains("Fetching")
        && !shippingCost.contains("-")){
      print(shippingCost);
      emeralds += double.parse(shippingCost).toInt();
    }
    return emeralds;
  }
  ...
  void _fetchCost() async{

    print(putAddress.country.toLowerCase());


    try{


    if(putAddress.country.toLowerCase() != "COLOMBIA" && putAddress.country.toLowerCase() != "colombia") {
      getMerchant(productsList.first.uid).then((value) async {
        setState(() {
          shippingCost = "Fetching...";
        });
        shippingCost = await getDHLQoute(
            value.storeLocation.country.characters.take(2).string
      , value.storeLocation.zipCode,
            putAddress.country, putAddress.zipCode);
        setState(() {});
      });
    }else{
      setState(() {
